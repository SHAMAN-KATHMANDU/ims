...
- **Compose:** See **[QUICK-REFERENCE.md](QUICK-REFERENCE.md)** for a minimal docker-compose example that uses these images.
...
- Add a `test` script to `apps/api` and `apps/web` in `package.json` so CI runs real tests (see **[../deployment/CI-CD-DOCUMENTATION.md](../deployment/CI-CD-DOCUMENTATION.md)**).
