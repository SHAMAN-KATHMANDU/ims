...
- Visual builder: [`AutomationFlowCanvas.tsx`](../../apps/web/features/automation/components/AutomationFlowCanvas.tsx) (linear reorder + branching preview when `AUTOMATION_BRANCHING` is on); authoring surface: [`AutomationBranchingAuthoringPanel.tsx`](../../apps/web/features/automation/components/AutomationBranchingAuthoringPanel.tsx) under **All fields** (single `flowGraph` source of truth; see §10.10)
...
| AT-UI-002 (partial)              | Flow canvas starts branching; **All fields** hosts `AutomationBranchingAuthoringPanel` (routing timeline + composable segments); Flow chart = preview only; not full freeform DAG                                                                        |
...
| AT-UI-002                        | E2E same file — if/switch buttons; `data-testid="automation-branching-authoring"` under **All fields**; `data-testid="automation-branching-flow-preview"` on Flow chart during branching                                                                 |
...
### 10.10 Phase 4 UI — linked form, composable timeline, DAG editor stance

| Topic                            | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Single mental model**          | One React Hook Form graph (`preservedBranchingFlowGraph`) drives both **All fields** (routing timeline) and **Flow chart** (read-only live preview while authoring). No duplicate authoring mounts.                                                                                                                                                                                                                                                                                                                                      |
| **Composable routing (Phase B)** | `compileComposableFlowSegments` / `tryExtractComposableFlowSegments` in [`packages/shared/src/automation/automation-flow-graph.ts`](../../packages/shared/src/automation/automation-flow-graph.ts) compile a **linear list of segments** (actions, canonical `if`/`switch` blocks) into a valid DAG; shared tests cover round-trip and `parseAndValidateAutomationFlowGraph`. UI timeline enforces **BR-11** via `composableSegmentsFitBr11Limits` / `estimateComposableSegmentsFootprint` (disable add with explanation when over cap). |
| **AT-UI gap vs full DAG**        | v1 remains **no arbitrary merge/join** and **no freeform node graph** in product UI. OR across branches still requires multiple `if` nodes per **§3** (surfaced in timeline help text).                                                                                                                                                                                                                                                                                                                                                  |
| **Phase C (optional spike)**     | A full **React Flow** palette (drag/connect `entry` / `action` / `if` / `switch` / `noop`) is **deferred** until composable timeline proves insufficient in product review; reuse existing graph types and `parseAndValidateAutomationFlowGraph` when/if implemented.                                                                                                                                                                                                                                                                    |

**Traceability:** composable compile/extract and BR-11 estimates — `packages/shared/src/automation/automation-flow-graph.test.ts`; branching panel unit test — `AutomationBranchingAuthoringPanel.test.tsx`; E2E **AT-UI-002** — `e2e/automation/event-automations.spec.ts` (`automation-branching-authoring` on All fields, `automation-branching-flow-preview` on Flow chart).
