...
| `sales`                  | covered     | sale created, high value created, deleted            | `apps/api/src/modules/sales/sale.service.ts`                                                                   | work items, notifications, webhooks, field update                 | `sale.service.test.ts` (asserts `sales.sale.created`, `high_value_created`, `deleted`) |
...
| Replay failed events     | covered | full replay and “resume failed steps” (`reprocessFromStart: false`) from recent runs when `automationEventId` is present |
