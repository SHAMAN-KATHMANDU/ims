...
# =============================================================================
# deploy/dev/logs.sh
# View logs for dev/stage containers.
#
# Usage:
#   ./logs.sh                  -- last 100 lines from all services
#   ./logs.sh api              -- last 100 lines from dev_api
#   ./logs.sh web              -- last 100 lines from dev_web
#   ./logs.sh db               -- last 100 lines from dev_db
#   ./logs.sh redis            -- last 100 lines from dev_redis
...
if [[ -z "$TARGET" ]]; then
  # All services from main compose
  ...
  echo ""
  echo "Valid options: api, web, db, redis, watchtower (or omit for all)"
  exit 1
fi
