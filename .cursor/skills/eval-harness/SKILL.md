...
const grader = {
  type: "exact_match",
  grade: (output: string, expected: string) =>
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an evaluator. Rate the output as PASS or FAIL based on the criteria.",
        },
        {
          role: "user",
...
async function runEvalSuite(suite: EvalSuite): Promise<EvalReport> {
  ...
  for (const evalCase of suite.cases) {
    ...
    try {
      ...
      const passed = await suite.grader.grade(
        output,
        evalCase.expected as string,
      );
...
async function evalPassAtK(suite: EvalSuite, k: number): Promise<number> {
  const scores = await Promise.all(
    suite.cases.map((c) =>
      passAtK(c, k, suite.grader, suite.model ?? "gpt-4o"),
    ),
  );
