 ...
 * Prefer `node prisma/seed.js --orchestrated` (sets SEED_ORCHESTRATED=1 before TS loads).
 * Optional env from the host/container `.env` is unchanged; deploy scripts also inline:
 *
 ...
 *   SEED_MINIMAL_TENANTS_B64 - optional UTF-8 CSV as base64: slug:Name or slug:Name:password
 *   SEED_MINIMAL_TENANTS   - optional plain CSV (local/CI); deploy uses B64 for safe shell passing
 *   SEED_TENANT_PASSWORD   - default password for minimal entries when omitted in list
 *   SEED_MODE              - development | production (logged; prod seed.sh uses production)
 *
 ...
 * Omit `--orchestrated` and keep SEED_ORCHESTRATED unset or not "1". Optional `node prisma/seed.js --legacy` clears SEED_ORCHESTRATED if your shell exported it.
 *
