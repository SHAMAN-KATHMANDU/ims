...
export async function login(
  ...
  if (!fullUrl.match(/^https?:\/\//)) {
    throw new Error(
      `Invalid API URL: ${fullUrl}. Must be an absolute URL starting with http:// or https://`,
    );
  }
  ...
  try {
    ...
    if (response.url !== fullUrl && !response.url.includes("localhost:4000")) {
      ...
      throw new Error(
        `Request was redirected. Expected API at localhost:4000 but got: ${response.url}`,
      );
    }
    ...
    console.log(
      "📄 Response body (raw, first 200 chars):",
    ...
    if (err.name === "TypeError" && err.message?.includes("fetch")) {
      throw new Error(
        `Cannot connect to server at ${fullUrl}. Please check if the API is running on port 4000.`,
      );
    }
    throw error;
  }
