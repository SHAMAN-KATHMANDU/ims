 ...
 */
export const calculateDiscountedPrice = (mrp: number, discountPercentage: number): number => {
...
export const getTotalStock = (product: Product): number => {
  ...
  return product.variations.reduce((sum, variation) => sum + (variation.stockQuantity || 0), 0)
}
  ...
  discounts.forEach(discount => {
    if (discount.isActive && discount.discountType?.name) {
      ...
      prices[typeName] = {
        ...
        price: calculateDiscountedPrice(product.mrp, discount.discountPercentage)
      }
    }
  })
 ...
 */
export const getCategoryName = (id: string, product?: Product, categories?: Array<{ id: string; name: string }>) => {
...
export const filterProducts = (
  ...
  return products.filter((product) => {
    const categoryName = getCategoryNameFn(product.categoryId, product).toLowerCase()
