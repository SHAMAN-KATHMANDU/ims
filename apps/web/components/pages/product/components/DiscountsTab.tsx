...
export function DiscountsTab({ products }: DiscountsTabProps) {
  ...
  const specialDiscounts = allDiscounts.filter(d => 
    d.discountType?.name?.toLowerCase() === "special" && d.isActive
  )
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Showing only special discounts. Edit a product to add or modify discounts.
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Showing only special discounts from all products. To add/edit discounts, edit the product.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
