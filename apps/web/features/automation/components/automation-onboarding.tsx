  ...
  return (
    <div className="space-y-4">
      <Alert className="border-primary/25 bg-primary/5">
        ...
        <AlertTitle>What automations do</AlertTitle>
        <AlertDescription className="space-y-2 text-muted-foreground">
          <p>
            ...
            <code className="rounded bg-muted px-1">inventory.*</code>), applies
            optional <strong>conditions</strong> and delays, then runs{" "}
            <strong>steps in order</strong> (create work items, CRM updates,
            webhooks, and more).
          </p>
          <p>
            <strong>LIVE</strong> runs real actions. <strong>SHADOW</strong>{" "}
            records what would happen without executing side effects—use it to
            validate triggers and steps, then check <strong>Recent runs</strong>{" "}
      ...
      <Accordion type="multiple" className="rounded-lg border px-4">
        <AccordionItem value="how">
          ...
          <AccordionContent className="space-y-2 text-sm text-muted-foreground">
            <p>
              Each <strong>trigger</strong> subscribes to one event name. You
              can narrow it with <strong>conditions</strong> on fields in the
              event payload and optionally delay execution by minutes.
            </p>
            <p>
              <strong>Steps</strong> run sequentially. Later steps can use
              outputs from earlier ones. Enable{" "}
              <strong>Continue on error</strong> on a step if you want following
              steps to run even when that step fails.
            </p>
          </AccordionContent>
        </AccordionItem>
        <AccordionItem value="setup">
          ...
          <AccordionContent className="space-y-2 text-sm text-muted-foreground">
            <ol className="list-decimal space-y-2 pl-5">
              ...
              <li>
                Add one or more <strong>triggers</strong> with the right event
                and optional conditions.
              </li>
              <li>
                Add <strong>steps</strong> (actions) in the order they should
                run.
              </li>
              <li>
                Set execution mode to <strong>SHADOW</strong>, save, and
                generate real events in your workspace; confirm behavior in{" "}
        ...
        <AccordionItem value="vs-workflows">
          ...
          <AccordionContent className="space-y-2 text-sm text-muted-foreground">
            <p>
              ...
              pipeline. They react to deal lifecycle moments (stage enter/exit,
              deal created/won/lost, etc.) and are ideal for sales process
              automation.
            </p>
            <p>
              <strong>Automations</strong> react to broader platform events
              across CRM, sales, inventory, and work items. Use them when the
              trigger is not limited to a single pipeline.
            </p>
            <p>
              If both could fire for the same deal, you can enable{" "}
              <strong>Suppress legacy CRM workflows</strong> on an automation so
              matching legacy pipeline workflows do not run for that event.
            </p>
          </AccordionContent>
        </AccordionItem>
      </Accordion>
